      ...
      <div style={{ display: isCropping ? 'none' : 'block' }}>
        ...
        {processedImages.length > 0 && (
            <div className="card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    <h3>Results</h3>
                    <button className="btn btn-primary" onClick={downloadAll}>Download All</button>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
                    {processedImages.map((img, idx) => (
                        <div key={idx} style={{ border: '1px solid var(--border)', borderRadius: '0.5rem', padding: '1rem' }}>
                            ...
                            <button className="btn btn-secondary" onClick={() => saveAs(img.blob, img.name)} style={{ width: '100%', marginTop: '0.5rem' }}>Download</button>
                        </div>
                    ))}
                </div>
            </div>
