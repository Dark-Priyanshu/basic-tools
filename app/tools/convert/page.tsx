    ...
    for (const image of images) {
      try {
        ...
        const blob = await new Promise<Blob>((resolve) => {
          if (format === 'ico') {
            // For ICO, resize to 32x32 standard for icons unless user specified size
            // If user specified size, we use that canvas (which is already resized). 
            // NOTE: ICO usually implies multiple sizes or specific icon sizes. 
            // For this simple tool, if user asked to resize, we give them that size in ICO format container (typically just PNG inside).
            // But to be safe and consistent with previous behavior for defaults:
            ...
            if (icoCtx) {
              // We need to draw from the MAIN canvas (which is already resized) if we want to keep that quality?
              // Or just draw from source img again? Drawing from source is better if we are changing size again.
